...
let rec double_true_help acc ls pos=
	...
	Graphics.set_color 0xfcf646; (*yellow*)
		...
		|hd::t->	begin
							match hd with
								|"up"-> 
										Graphics.fill_poly [|(x,y+h);(x+100,y+h);(x+50,y+h+100)|];
										double_true_help ((pos.x,pos.y+pos.h)::acc) t pos
								|"down"-> Graphics.fill_poly [|(x,y-10);(x+100,y-10);(x+50,y-110)|];
										double_true_help ((pos.x,pos.y-1)::acc) t pos
								|"left"-> Graphics.fill_poly [|(x-10,y);(x-10,y+100);(x-110,y+50)|];
										double_true_help ((pos.x-1,pos.y)::acc) t pos
								|"right"-> Graphics.fill_poly [|(x+w,y);(x+w,y+100);(x+w+100,y+50)|];
...
let four_direction_check x y w h ls= 
	...
	let down_check= if (find_name (x,y-1) ls)="E"&&(find_name (x+w-1,y-1) ls)="E" then "down" else "f" in
	let right_check= if (find_name (x+w,y) ls)="E"&&(find_name (x+w,y+h-1) ls)="E" then "right" else "f" in
	let left_check= if (find_name (x-1,y) ls)="E"&&(find_name (x-1,y+h-1) ls)="E" then "left" else "f" in 
...
let get_box_number x y =
	...
	else if ((x+80)/110<>(x-20)/110)&&((y+80)/110<>(y-20)/110) then ((x+80)/110-1,(y+80)/110-1) 
	else (5,5) (*oustide*) 

let make_new_ls x y puzzle=
	Printf.printf "mouse is: %d\t%d\n" x y;	
	if x>130&&x<360&&y>130&&y<470&&((x-21)/110<>(x-30)/110)&&((y-21)/110<>(y-30)/110)(*cross section*)
		then begin	
					Printf.printf "%s\n" "cross section";
	...
	else if x>130&&x<360&&y>30&&y<570&&((x-21)/110<>(x-30)/110) (*straight lines*) 
		then begin 
					Printf.printf "%s\n" "straight lines";
	...
	else if x>30&&x<360&&y>130&&y<470&&((y-21)/110<>(y-30)/110)(*horizontal lines*)
		then begin
					Printf.printf "%s\n" "horizontal lines";
