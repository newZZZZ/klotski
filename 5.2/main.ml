...
let rec check_moveable i ls=
	...
	if name="E" then ls
		...
		begin
			...
			(*10 directions check*)
			let judge=List.filter (fun x -> x<>Fail) (four_direction_check pos.x pos.y pos.w pos.h ls) in
...
let make_new_ls x y puzzle=
	...
	else if (x>=button_start_x&&x<=(button_start_x+button_length)&&y>=(button_start_y+button_gap)&&y<=button_gap*2) (*Stage AutoSolve*)then Some stage_one  
